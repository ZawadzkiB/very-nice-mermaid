import { test, expect, type Page, type ConsoleMessage } from "@playwright/test";
import { exportHtml } from "./helpers.js";

/**
 * Drives the standalone exported HTML for a NATIVE class diagram over file:// —
 * the artifact from `vnm render shop-class.mmd -o out.html`. Class diagrams are
 * node-graphs, so they reuse the flowchart vnmRuntime: full draggable nodes with
 * live edge re-routing + pan/zoom/fit. Requires a Chromium install
 * (`npx playwright install chromium`).
 */

let url: string;
test.beforeAll(() => {
  url = exportHtml("shop-class.mmd", "light", "exported-class.html");
});

test.beforeEach(async ({ page }) => {
  await page.goto(url);
  await page.waitForSelector(".vnm-node");
});

const worldTransform = (page: Page) =>
  page.locator(".vnm-world").evaluate((el) => (el as HTMLElement).style.transform);

test("renders class node cards carrying their member/method text", async ({ page }) => {
  const texts = await page.locator(".vnm-node").allInnerTexts();
  const joined = texts.join(" ");
  expect(joined).toContain("Animal");
  expect(joined).toContain("+String name");
  expect(joined).toContain("makeSound");
});

test("dragging a class node re-routes its relation edges", async ({ page }) => {
  const edges = page.locator("svg.vnm-edges path[marker-end], svg.vnm-edges path[marker-start]");
  const before = await edges.evaluateAll((ps) => ps.map((p) => p.getAttribute("d")));

  // Dog is central (5 relations touch it), so dragging it re-routes several.
  const node = page.locator(".vnm-node", { hasText: "Dog" }).first();
  const box = (await node.boundingBox())!;
  await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
  await page.mouse.down();
  await page.mouse.move(box.x + 160, box.y + 120, { steps: 8 });
  await page.mouse.up();

  const after = await edges.evaluateAll((ps) => ps.map((p) => p.getAttribute("d")));
  expect(after).not.toEqual(before);
});

test("wheel zooms and background drag pans the view", async ({ page }) => {
  const before = await worldTransform(page);
  await page.locator(".vnm-viewport").hover({ position: { x: 220, y: 180 } });
  await page.mouse.wheel(0, -300);
  const zoomed = await worldTransform(page);
  expect(zoomed).not.toBe(before);

  await page.mouse.move(180, 160);
  await page.mouse.down();
  await page.mouse.move(320, 260, { steps: 8 });
  await page.mouse.up();
  const panned = await worldTransform(page);
  expect(panned).not.toBe(zoomed);
});

test("a full interaction session produces no console errors", async ({ page }) => {
  const errors: string[] = [];
  page.on("console", (msg: ConsoleMessage) => {
    if (msg.type() === "error") errors.push(msg.text());
  });
  page.on("pageerror", (err) => errors.push(String(err)));

  const node = page.locator(".vnm-node").first();
  const nb = (await node.boundingBox())!;
  await page.mouse.move(nb.x + nb.width / 2, nb.y + nb.height / 2);
  await page.mouse.down();
  await page.mouse.move(nb.x + 120, nb.y + 90, { steps: 6 });
  await page.mouse.up();
  await page.locator(".vnm-viewport").hover({ position: { x: 220, y: 180 } });
  await page.mouse.wheel(0, -240);

  await page.waitForTimeout(50);
  expect(errors).toEqual([]);
});
